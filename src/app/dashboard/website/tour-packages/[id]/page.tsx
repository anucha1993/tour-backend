...
import { tourPackagesApi, countriesApi, Country, TourPackage } from '@/lib/api';
...
export default function TourPackageEditPage() {
  ...
  const fetchPackage = useCallback(async () => {
    setLoading(true);
    try {
      const res = await tourPackagesApi.get(id);
      const data = (res as any)?.data;
      if (data) {
        setPkg(data);
        setName(data.name || '');
        ...
        setInclusions(data.inclusions?.length > 0 ? data.inclusions : ['']);
        setExclusions(data.exclusions?.length > 0 ? data.exclusions : ['']);
        setTimeline(data.timeline?.length > 0 ? data.timeline : [{ day_number: 1, detail: '' }]);
        setHashtags(data.hashtags || []);
        setIsNeverExpire(data.is_never_expire);
        ...
        setSelectedCountryIds(data.countries?.map((c: any) => c.id) || []);
      }
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await tourPackagesApi.update(id, payload);
      router.push('/dashboard/website/tour-packages');
    } catch (error) {
      console.error('Failed to update:', error);
      alert('เกิดข้อผิดพลาดในการอัปเดตแพ็คเกจ');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleUploadImage = async (file: File) => {
    ...
    try {
      const res = await tourPackagesApi.uploadImage(id, file);
      if (res?.data) fetchPackage();
    } catch (e) { console.error(e); }
  ...
  const handleDeleteImage = async () => {
    ...
    try {
      await tourPackagesApi.deleteImage(id);
      fetchPackage();
    } catch (e) { console.error(e); }
  };
  ...
  const handleUploadPdf = async (file: File) => {
    ...
    try {
      const res = await tourPackagesApi.uploadPdf(id, file);
      if (res?.data) fetchPackage();
    } catch (e) { console.error(e); }
  ...
  const handleDeletePdf = async () => {
    ...
    try {
      await tourPackagesApi.deletePdf(id);
      fetchPackage();
    } catch (e) { console.error(e); }
  };
  ...
  if (!pkg) {
    return (
      <div className="text-center py-20">
        ...
        <h2 className="text-lg font-semibold text-gray-700">ไม่พบแพ็คเกจทัวร์</h2>
        <Link href="/dashboard/website/tour-packages" className="text-blue-500 hover:underline mt-2 inline-block">
          กลับหน้ารายการ
        </Link>
      </div>
    );
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-500 mt-0.5 text-sm">{pkg.name}</p>
          </div>
        </div>
          ...
          <div className="px-5 pb-5 space-y-4 border-t border-gray-100 pt-4">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <div>
                ...
                />
                <p className="text-xs text-gray-400 mt-1">ใช้สำหรับ URL เช่น /tours/packages/{slug || 'your-slug'}</p>
              </div>
            </div>
              ...
              {selectedCountryIds.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mb-2">
                  {selectedCountryIds.map(cid => {
                    const c = allCountries.find(cx => cx.id === cid);
                    return c ? (
                      <span key={cid} className="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-sm bg-blue-50 text-blue-700 border border-blue-100">
                        {c.flag_emoji} {c.name_th || c.name_en}
                        <button onClick={() => setSelectedCountryIds(prev => prev.filter(x => x !== cid))} className="hover:text-red-500">
                ...
                <div className="relative w-64 h-64 rounded-lg overflow-hidden bg-gray-100 group">
                  ...
                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors flex items-center justify-center opacity-0 group-hover:opacity-100">
                    <div className="flex gap-2">
                      ...
                      >
                        {uploadingImage ? <Loader2 className="w-4 h-4 animate-spin" /> : 'เปลี่ยน'}
                      </button>
                      <button
                ...
                >
                  {uploadingImage ? (
                    ...
                    <div className="flex flex-col items-center">
                      ...
                      <span className="text-sm text-gray-500">คลิกเพื่ออัปโหลดรูปภาพปก</span>
                    </div>
                  )}
                </button>
              )}
              ...
              {pkg.pdf_url ? (
                <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg max-w-md">
                  ...
                  <div className="flex-1 min-w-0">
                    <a href={pkg.pdf_url} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-600 hover:underline truncate block">
                      ดูไฟล์ PDF
                    </a>
                  </div>
                  <div className="flex gap-2 shrink-0">
                    ...
                    >
                      {uploadingPdf ? <Loader2 className="w-3 h-3 animate-spin" /> : 'เปลี่ยน'}
                    </button>
                    <button
                ...
                >
                  {uploadingPdf ? (
                    ...
                    <div className="flex flex-col items-center">
                      ...
                      <span className="text-sm text-gray-500">คลิกเพื่ออัปโหลดไฟล์ PDF</span>
                    </div>
                  )}
                </button>
              )}
      ...
      <Card className="overflow-hidden">
        ...
        {sectionOpen.content && (
          <div className="px-5 pb-5 space-y-6 border-t border-gray-100 pt-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">เงื่อนไขการให้บริการ</label>
              <QuillEditor
                value={terms}
                onChange={setTerms}
             ...
             <div>
              <label className="block text-sm font-medium text-gray-700">เงื่อนไขในการยกเลิกทัวร์</label>
              <QuillEditor

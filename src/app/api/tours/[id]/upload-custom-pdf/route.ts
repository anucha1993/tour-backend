...
export async function POST(
  ...
  try {
    ...
    // Re-create a clean FormData to forward upstream
    const upstreamForm = new FormData();
    for (const [key, value] of formData.entries()) {
      upstreamForm.append(key, value);
    }

    const upstream = await fetch(`${apiTarget}/tours/${id}/upload-custom-pdf`, {
      method: 'POST',
      headers: {
    ...
    if (!respContentType.includes('application/json')) {
      ...
      console.error(`[upload-custom-pdf] upstream ${upstream.status}:`, text.slice(0, 1000));
    ...
    console.error('[upload-custom-pdf] route handler error:', err);
    return NextResponse.json(

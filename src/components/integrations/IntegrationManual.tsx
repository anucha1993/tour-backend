...
const BOOKING_PROVIDERS: { code: string; name: string; api: string; note: string }[] = [
  { code: 'zego', name: 'Wholesale — Custom Booking API', api: 'REST v1.5 (3 ขั้นตอน)', note: 'ยิงจองเข้าระบบ Wholesale โดยตรงแบบเรียลไทม์' },
...
const ZEGO_CONFIG_FIELDS: Field[] = [
  ...
  { key: 'max_pax', label: 'ผู้โดยสารสูงสุด/การจอง', type: 'number', required: false, note: 'ค่าเริ่มต้น 10 (Wholesale จำกัด 10 คน)' },
];
...
const SAMPLE_QUOTE_RES = `// QuoteResult (toArray)
{
  ...
  "is_real_hold": false                  // Wholesale = pseudo-hold (นับถอยหลังฝั่งเรา)
}`;

const SAMPLE_BOOK_REQ = `POST /api/bookings/outbound/{id}/confirm
{
  "customer_name":  "สมชาย ใจดี",
  "customer_phone": "0812345678",        // Wholesale บังคับ 10 หลักพอดี
...
failed    → ยิงจองไม่สำเร็จ (แอดมินทำมือต่อ หรือกด retry)`}</CodeBlock>
          ...
          <SectionCard id="ob-providers" icon={<Server className="w-5 h-5" />} title="B. Provider ที่รองรับ + ความสามารถ">
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50 text-gray-600">
                  <tr>
                    ...
                    <th className="text-center px-3 py-2 font-medium">Wholesale</th>
            ...
            <div className="bg-amber-50 border border-amber-100 rounded-lg p-3 text-xs text-amber-800">
              <strong>real_hold = ไม่รองรับทั้งคู่:</strong> ทั้ง Wholesale และ Manual เป็น &ldquo;pseudo-hold&rdquo; —
          ...
          <SectionCard id="ob-config" icon={<KeyRound className="w-5 h-5" />} title="C. การตั้งค่าต่อ Integration">
            ...
            <h3 className="font-semibold text-gray-900 mt-3 flex items-center gap-1.5">
              <Send className="w-4 h-4 text-blue-600" /> C.2 ฟิลด์ของ Wholesale (<code className="text-xs">zego</code>)
            </h3>
...
4) cancel   ── ยกเลิก (เฉพาะ provider ที่รองรับ)`}</CodeBlock>
            ...
            <div className="bg-amber-50 border border-amber-100 rounded-lg p-3 text-xs text-amber-800">
              ...
              (เช่น Wholesale ใช้ MA=ผู้ใหญ่, CH=เด็ก, IF=ทารก) — อย่าเดาเอง
            </div>
          </SectionCard>
          ...
          <SectionCard id="ob-provider-apis" icon={<Braces className="w-5 h-5" />} title="E. รายละเอียด API แต่ละเจ้า">
            <h3 className="font-semibold text-gray-900 flex items-center gap-1.5">
              <Send className="w-4 h-4 text-blue-600" /> E.1 Wholesale — Custom Booking API v1.5 (3 ขั้นตอน)
            </h3>
            <p className="text-xs text-gray-500">
            ...
            <ul className="list-disc ml-5 space-y-1 text-xs text-gray-600">
              ...
              <li>ไม่รองรับ cancel/modify ฝั่ง Wholesale — ต้องติดต่อ Wholesaler โดยตรง</li>
          ...
          <SectionCard id="ob-partner" icon={<ShieldCheck className="w-5 h-5" />} title="F. สำหรับ Wholesaler ที่มี Booking API">
            <p>
              ถ้าคุณมี Booking API เป็นของตัวเอง เราสร้าง provider adapter ให้ได้ (แบบเดียวกับ Wholesale)
            ...
            <div className="bg-emerald-50 border border-emerald-100 rounded-lg p-3 text-xs text-emerald-800">
              ...
              แต่ละเจ้าอยู่ใน adapter — เพิ่ม Wholesaler ใหม่ที่ใช้ API เดิม (เช่น Wholesale) ทำได้ทันทีโดยไม่ต้องเขียนโค้ด
            </div>
          </SectionCard>
        </div>
      </div>
